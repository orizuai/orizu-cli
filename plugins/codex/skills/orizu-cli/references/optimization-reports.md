...
7. Write a recommendation and named next move for every scenario that did not improve or regressed. When the move changes examples, coverage, labels, or split design, return to **J3 — Build the dataset** rather than changing instructions without measurement.
...
- **Add coverage — J3 — Build the dataset:** collect or create more examples for this class, create a new dataset version, revisit split and optimizer-batch coverage, then rerun optimization.
- **Repair ground truth — J3 — Build the dataset, then conditional J4 — Ground truth via annotation:** send named contested rows for human re-review, correct rationales or labels, create a new dataset version, and revalidate the judge before optimizing again.
- **Improve evaluator feedback — J5 — Build & validate judges:** make the scorer explain the concrete disagreement, revalidate it, then rerun.
- **Change the application — J2 — Assess & plan:** when the failure needs retrieval, tools, state, or code rather than instructions, return to the ratified improvement plan and name the engineering change.
- **Guard a regression — J3 — Build the dataset:** add the regressed rows and nearby counterexamples to a new dataset version, preserve them in validation, then rerun candidate selection.
...
When a class is absent or thin in training, the optimizer cannot learn it from reflection. Return to J3 — Build the dataset. When labels or rationales are contested, repair ground truth and create a new dataset version before rerunning. When the scorer gives vague feedback or disagrees with human judgment, return to J5 — Build & validate judges. When the system needs tools, state, retrieval, or code, return to J2 — Assess & plan rather than forcing the behavior into instructions.

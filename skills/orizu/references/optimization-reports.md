...
# Agent-run on all supported surfaces: export report evidence, then inspect whether this identity serves a production profile or the set default.
...
7. Write a recommendation and named next move for every scenario that did not improve or regressed. When the move changes examples, coverage, labels, or split design, return to First win's dataset-curation stage rather than changing instructions without measurement.
...
| --- | --- | --- | --- | --- | --- |
| [class] | [user/system consequence] | [training row count / validation row count] | [fixed, unchanged failures, regressions, and row ids] | [plain-language lesson] | [named action, owner, and flow or stage] |

For each class that did not improve or regressed, choose and tailor a named move:

- **Add coverage — First win / dataset curation:** collect or create more examples for this class, create a new dataset version, revisit split and optimizer-batch coverage, then rerun optimization.
- **Repair ground truth — First win / dataset curation, then conditional annotation:** send named contested rows for human re-review, correct rationales or labels, create a new dataset version, and revalidate the judge before optimizing again.
- **Improve evaluator feedback — First win / judge validation:** make the scorer explain the concrete disagreement, revalidate it, then rerun.
- **Change the application — assess and plan:** when the failure needs retrieval, tools, state, or code rather than instructions, return to the ratified improvement plan and name the engineering change.
- **Guard a regression — First win / dataset curation:** add the regressed rows and nearby counterexamples to a new dataset version, preserve them in validation, then rerun candidate selection.
...
2. **[Scenario class] — [owner]:** [Specific dataset, annotation, judge, or application action; artifact to create; flow or stage to resume.]
...
When a class is absent or thin in training, the optimizer cannot learn it from reflection. Return to First win's dataset-curation stage. When labels or rationales are contested, repair ground truth and create a new dataset version before rerunning. When the scorer gives vague feedback or disagrees with human judgment, return to First win's judge-validation stage. When the system needs tools, state, retrieval, or code, return to the ratified assess-and-plan procedure rather than forcing the behavior into instructions.

...
export const PREVIEW_ALLOWED_IMPORTS: Record<string, string[]> = {
...
interface PreviewPlaywright {
  chromium: {
    launch: (options: Record<string, unknown>) => Promise<PreviewBrowser>
  }
}

interface PreviewBrowser {
  newPage: (options: Record<string, unknown>) => Promise<PreviewPage>
  close: () => Promise<void>
...
export async function runLocalAppPreview(options: AppPreviewOptions): Promise<AppPreviewResult> {
  ...
  try {
    ...
    if (!esbuild) {
      throw new Error(
        'Local preview requires esbuild. Install project dependencies with `bun install` or run from an Orizu checkout.'
      )
    }
...
function waitForPreviewClose(page: PreviewPage, browser: PreviewBrowser): Promise<void> {
  return new Promise(resolveClose => {
    ...
    const listen = (target: PreviewPage | PreviewBrowser, event: string) => {
...
function validateImports(source: string, issues: Issue[]) {
...
function validateDefaultExport(source: string, componentName: string | undefined, issues: Issue[]) {
...
function resolveDefaultExport(source: string): { name: string; params: string | null } | null {
  const directFunction = source.match(
    /export\s+default\s+function\s+([A-Za-z_$][\w$]*)\s*\(([\s\S]*?)\)/
  )
  if (directFunction?.[1]) {
  ...
  const functionDeclaration = source.match(
    new RegExp(`function\\s+${escapedIdentifier}(?![A-Za-z0-9_$])\\s*\\(([\\s\\S]*?)\\)`)
  )
  ...
  const arrowDeclaration = source.match(
    new RegExp(`(?:const|let|var)\\s+${escapedIdentifier}(?![A-Za-z0-9_$])\\s*=\\s*(?:\\(([^)]*)\\)|([^=()\\n]+))\\s*(?::\\s*[^=\\n]+)?\\s*=>`)
  )
  if (arrowDeclaration) {
  ...
  const classDeclaration = source.match(
    new RegExp(`class\\s+${escapedIdentifier}(?![A-Za-z0-9_$])`)
  )
  if (classDeclaration) {
...
function validateSchema(schema: unknown, label: string, issues: Issue[], path = label) {
...
function validatePayload(schema: unknown, payload: unknown, path: string, issues: Issue[]) {
  ...
  if (obj.type === 'object') {
    ...
    if (obj.properties && typeof obj.properties === 'object' && !Array.isArray(obj.properties)) {
      for (const [key, child] of Object.entries(obj.properties)) {
        if (key in record) validatePayload(child, record[key], `${path}.${key}`, issues)
      }
    }
  }
  if (obj.type === 'array') {
...
function findCliPackageRoot(): string | null {
...
function buildEntrySource(appPath: string, sampleRow: Record<string, unknown>) {
  return `
...
function buildPreviewSnapshotModules(): Record<string, string> {
...
export function getPreviewSnapshotModulePaths(): string[] {
  return Object.keys(buildPreviewSnapshotModules()).sort()
}

async function serveDirectory(root: string): Promise<{ port: number; close: () => Promise<void> }> {
...
async function listenOnAvailablePort(server: ReturnType<typeof createServer>): Promise<number> {

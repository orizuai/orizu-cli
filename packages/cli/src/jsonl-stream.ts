...
function rowsPayloadByteLength(rowCount: number, rowsByteLength: number): number {
...
function mapReadError(filePath: string, error: unknown): Error {
...
export function estimateRowsPayloadByteLength(rows: Array<Record<string, unknown>>): number {
...
export async function* streamJsonlRowChunks(
  filePath: string,
  options: JsonlChunkOptions = {}
  ...
  try {
    for await (const line of lines) {
      ...
      if (singleRowPayloadBytes > maxPayloadBytes) {
        throw new Error(
          `Row ${lineNumber} is ${formatMb(singleRowPayloadBytes)} MB after JSON encoding, which exceeds the ${formatMb(maxPayloadBytes)} MB upload chunk limit`
        )
      }

      const nextPayloadBytes = rowsPayloadByteLength(
        chunk.length + 1,

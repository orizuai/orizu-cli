...
function compactTerminalText(value, maxLength = 160) {
...
function printIndentedBody(value, indent) {
    const lines = sanitizeTerminalText(value || '').split(/\r?\n/);
    for (const line of lines) {
        printLine(`${indent}${line}`);
    }
}
function formatPromptCommentAnchor(anchor) {
    if (!anchor) {
        return null;
    }
...
function printPromptComments(data) {
    const versionLabel = data.version.versionLabel || (data.version.versionNumber ? `v${data.version.versionNumber}` : data.version.id);
    ...
    printLine(`Comments: ${data.summary.threadCount} thread${data.summary.threadCount === 1 ? '' : 's'} ` +
...
async function listPromptComments() {
    ...
    const data = await parseJsonResponse(response, 'Prompt comments');
    if (hasJsonFlag()) {
        printJson(data);
        return;
    }
...
function looksLikeAggregateScoreObject(value) {
    return (value.scoreValue !== undefined ||
...
function readOptionalJsonlFile(pathArg) {
    if (!pathArg)
        return undefined;
...
function parseAliasAssignments(value) {
    const result = {};
    if (!value)
        return result;
...
function readDependencyResultAssignments(value) {
...
async function execScorer() {
    ...
    const data = await parseJsonResponse(response, 'Scorer exec');
    if (outPath) {
...
async function exportOptimizationRun() {
    ...
    const data = await parseJsonResponse(response, 'Optimization export');
...
async function createTask() {
    ...
    printLine(`Created ${isDraft ? 'draft ' : ''}task ${sanitizeTerminalText(data.task.title)} (${sanitizeTerminalText(data.task.id)}) [${sanitizeTerminalText(data.task.status)}]` +
...
async function publishTask() {
    ...
    const data = await parseJsonResponse(response, 'Task publish');
    if (hasArg('--json')) {
        printLine(JSON.stringify({
            taskId: data.task.id,
            status: data.task.status,
    ...
    printLine(`Published task ${sanitizeTerminalText(data.task.id)} [${sanitizeTerminalText(data.task.status)}]` +
        `\n  Assignments: ${data.assignmentsCreated}` +
        (data.assignmentsRemoved !== undefined
...
async function exportAppSource() {
    ...
    const data = await parseJsonResponse(response, 'App export');
    const filename = outPathArg
        ? expandHomePath(outPathArg)
        : defaultAppExportFilename(data.app.name, data.version.versionNum);
    writeTextFileEnsuringDir(filename, data.version.code);
    printLine(`Saved app ${sanitizeTerminalText(data.app.name)} v${data.version.versionNum} source to ${sanitizeTerminalText(filename)}`);
}

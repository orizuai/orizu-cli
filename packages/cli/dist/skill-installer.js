...
export function isSafeRelativeSkillPath(rel) {
    return rel !== '..' && !rel.startsWith('../') && !isAbsolute(rel);
}
export function formatMissingSkillSourceError(paths) {
...
export function getSkillInstallPath(target, options) {
...
function shouldCopySkillPath(sourceRoot, sourcePath) {
    ...
    return !parts.some(part => part === '.DS_Store' ||
        part === '__pycache__' ||
        part === '.pytest_cache') && !rel.endsWith('.pyc') && !rel.endsWith('.pyo');
}
...
function buildAgentsSection(sourceDir) {
...
function installAgentsMd(sourceDir, targetPath, options) {
...
export function targetNeedsOverwrite(target, options) {
...
export function isSkillInstallTarget(value) {
    return SKILL_INSTALL_TARGETS.includes(value);
}
